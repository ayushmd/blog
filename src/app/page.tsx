...
function SocialButton({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-[var(--surface)] text-[var(--foreground)] transition hover:bg-[var(--border)] hover:text-[var(--accent)]"
    >
      {children}
    </a>
  );
    ...
    <main className="mx-auto max-w-3xl px-3 py-10">
      <section className="mb-12">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <SocialButton href={site.socials.email} label="Email">
      ...
      <p className="mb-6 text-sm text-[var(--muted)]">
        Thoughts and notes in one place.
      </p>
      <ul className="space-y-6">
        {posts.length === 0 ? (
          <li className="text-[var(--muted)]">
            No posts yet. Add markdown files in <code className="rounded bg-[var(--border)] px-1.5 py-0.5 text-[var(--foreground)]">content/posts/</code>.
          </li>
        ) : (
          posts.map((post) => (
            <li key={post.slug}>
